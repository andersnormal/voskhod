...
var _DefaultVoskhodClientCommandConfig = _NewVoskhodClientCommandConfig()

type _VoskhodClientCommandConfig struct {
...
func _NewVoskhodClientCommandConfig() *_VoskhodClientCommandConfig {
	c := &_VoskhodClientCommandConfig{}
	envconfig.Process("", c)
	return c
}

func (o *_VoskhodClientCommandConfig) AddFlags(fs *pflag.FlagSet) {
...
func _DialVoskhod() (*grpc.ClientConn, VoskhodClient, error) {
	cfg := _DefaultVoskhodClientCommandConfig
	...
	return conn, NewVoskhodClient(conn), nil
}

type _VoskhodRoundTripFunc func(cli VoskhodClient, in iocodec.Decoder, out iocodec.Encoder) error

func _VoskhodRoundTrip(sample interface{}, fn _VoskhodRoundTripFunc) error {
	cfg := _DefaultVoskhodClientCommandConfig
...
Authenticate using the Authorization header (requires transport security):
	...
	echo '{json}' | registeragent --tls`,
	Run: func(cmd *cobra.Command, args []string) {
		var v RegisterAgentRequest
		err := _VoskhodRoundTrip(v, func(cli VoskhodClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			resp, err := cli.RegisterAgent(context.Background(), &v)
...
func init() {
	VoskhodClientCommand.AddCommand(_VoskhodRegisterAgentClientCommand)
	_DefaultVoskhodClientCommandConfig.AddFlags(_VoskhodRegisterAgentClientCommand.Flags())
}
